...
> **Status (pick-accuracy + calibration overhaul, 2026-07-04)**: Four coupled fixes to the real pick path. **(1) Camera-latency compensation:** vision frames were anchored to the belt position at *ingest* time though captured ~80–150 ms earlier (exposure+YOLO+poll), so the tracker believed objects were upstream by `v·Δt`. Now the capture thread stamps each frame's decode time (`image_processing.py` `_capture_loop`/`_emit_detections`, backdated by ½ `exposure_time_absolute`), `BeltPositionTracker` keeps a `(t, position)` ring buffer with `position_at(t)` (interp), and `PickScheduler.ingest_detections(..., position_at=)` anchors each detection to the belt position **at its capture instant** (falls back to `p_now`; no-op for the static/simulated belt). **(2) Oblique descent:** the pre_pick→pickup drop was vertical at a fixed R-frame XY, so contact happened with horizontal relative velocity = belt speed. The arm now **parks upstream** by `v·t_d` along `u_hat` (`ConveyorFrame.theta_rad`/`u_hat`, `_park_position`/`_descent_time_s`) and slants down to the contact point, tracking the object (zero relative velocity at contact); degenerates to vertical at belt speed 0. Gate lead now includes `plan.descend_time_s`; `[GATE]` log prints `t_d_model_s`. **(3) Rotation timeline:** `rotate_absolute` used to fire *between* gate-fire and pick dispatch (uncompensated round-trip on the critical path). New order: home the axis to logical 0 during goto flight → gate → pick → **after grip, once the arm lifts back to `z ≥ pre_pick`, rotate the attached board to normalise it to the bin orientation** (`rotate_deg = wrap_180(rotate_offset_deg − (vision_heading + theta))`). The PC/scheduler now speak a **logical angle in [-180,180)** (short-way turns); the IPC boundary (`main.py` worker + `EthernetCom.logical_to_physical_rotate`) remaps to the PLC's hardcoded physical [0,360) by a +180 shift (rotate_absolute only; status feedback mapped back). **Sign + `rotate_offset_deg` are unverified — calibrate on the first hardware run.** **(4) Calibration tooling:** new `modules/latency_probe.py` (`python3 -m modules.latency_probe [--target omron|siemens|both]`) times each PLC's status read separately (the IPC worker reads Omron then Siemens *sequentially*, so the old ~25 ms "round trip" was the sum; only Omron gates a pick) and suggests `ethernet_delay_s`; and a `[CONFIG-SUGGEST]` block at accuracy/acceptance scenario end (`EvaluateMetrics.config_suggestions`) reports `robot_movement_delay_s` = mean(measured wall − interpolator-modeled motion), `ethernet_delay_s` = round-trip avg, and measured vs configured XY speed. Compile + sim (throughput/accuracy) + realtime-plan + mock-PLC/probe smoke tests pass; the RealtimePickExecutor timeline itself needs hardware to exercise.
