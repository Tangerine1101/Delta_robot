...
# 4b. Interactive CLI against an in-process fake PLC (no hardware; ephemeral port)
...
# 7. NOTE: `production` can no longer be dry-run with --simulate-executor — it raises
#    "requires live PLC conveyor_position feedback" by design. Use test_throughput (cmd 2)
#    for the simulated pick pipeline, or run production on real hardware.
...
## 8. Config Key Reference (`modules/config.json`)

> Audited + pruned for deployment 2026-07-08: every key below is read by live code (audit of main.py + modules/*). Removed as dead: top-level `period_s`, `conveyor.length_mm`, `vision.capture.backend`, `scheduler.throughput_spawn_x`, `scheduler.accuracy_points` (legacy R-frame evaluate fallback — code fallback path retained), `vision.orientation.offset_deg`/`symmetry_deg` (shadowed: the `*_by_class` maps cover both classes; code fallback logic retained). Renamed: `test_conveyor_belt_speed_mm_s` → **`belt_speed_static_mm_s`**. `evaluate_stability_window_s`/`evaluate_stability_mm`/`evaluate_stability_arm_mm` are intentionally implicit (test-only; code defaults 0.4 s / 0.3 mm / 3.0 mm).
...
| `limit_radius_xy` | Physical reach circle (mm); enforced at `PLCGateway.send_package` (`EthernetCom.py`, `conveyor.py`, `scheduler.py`) |
| `object_types.{QFP,TQFP}` | Per-class `destination`/`thickness_mm`/`w`/`h` (`scheduler.py`) |
| `QFP`, `TQFP` (3-elem arrays) | R-frame drop positions, looked up dynamically via `object_types.*.destination` (`scheduler.py`) |
...
| `frame.theta_deg`, `frame.robot_origin_uv` | C-frame → R-frame transform (`conveyor.py`) |
...
| Key | Purpose |
|---|---|
| `model_weights`, `imgsz`, `conf`, `conf_marker`, `iou`, `device`, `half`, `class_map`, `show_window` | YOLO-OBB inference setup (`image_processing.py`) |
| `capture.{camera_usb_id,device,width,height,pixelformat,fps}` | PyAV camera capture (backend is hardwired to PyAV) |
| `v4l2_controls.*` | Applied verbatim via v4l2 (manual exposure for stable 30 fps); `exposure_time_absolute` also feeds latency backdating |
| `pixels_per_mm`, `roi.*`, `trigger_line.*` | Camera calibration / detection gating |
| `orientation.{enabled,pcb_classes,marker_map,cross_check,offset_by_class,symmetry_by_class,marker_max_dist_mm}` | Marker-based board heading (per-class offset/symmetry; global `offset_deg`/`symmetry_deg` fallbacks removed from config, code defaults 0.0/180 remain) |
| `tracker.*`, `belt_estimator.*` | Centroid tracker + informational belt-speed estimate |

### `scheduler`
| Key | Purpose |
|---|---|
| `home_position`, `clearance_height`, `slope_transition_height`, `pre_pick_height`, `pickup_height`, `place_height`, `corner_blend_xy` | Trajectory template geometry (height hierarchy §4.2) |
| `intercept_lead_time_s`, `release_descent_time_s` | Realtime park lead / place-phase timing |
| `nominal_xy_speed`, `nominal_z_speed` | Coarse constant-velocity ETA (feed-time estimate + logs) — NOT the trajectory timing model |
| `interpolator.{v_max,a_max,d_max,soft_start_s,scurve_shape_factor}` | Exact PLC S-curve trajectory-time model — this is what actually times segments |
| `stale_timeout_s`, `speed_timeout_s`, `poll_interval_s` | Tracker staleness / speed staleness / status poll cadence |
| `default_speed` | **Simulated** belt speed only (`SimulatedSpeedSource`); real belt uses `belt_speed_static_mm_s` |
| `robot_movement_delay_s`, `ethernet_delay_s` | Dispatch→motion latency model (gate lead); calibrate via `[GATE]` log / `latency_probe` |
| `rotate_offset_deg`, `rotate_sign`, `rotate_home_tolerance_deg`, `rotate_refresh_max_delta_deg` | Post-grip rotation normalisation chain (2026-07-06 blocks above) |
| `pick_arrival_tolerance_mm`, `pick_arrival_tolerance_max_mm` | RealtimePickExecutor arm-arrival tolerance, **speed-mapped 2026-07-09**: linear from the floor (`_mm`, at/below `belt_speed_min_mm_s`) to the ceiling (`_max_mm`, at/above `belt_speed_max_mm_s`), re-evaluated per wait-loop tick from live belt speed. `_max_mm` omitted or ≤ floor ⇒ static floor (pre-2026-07-09 behavior) |
| `belt_speed_static_mm_s` | Startup/static real-belt setpoint; adaptive controller's seed |
| `adaptive_speed_enabled`, `pick_cycle_s`, `pick_transit_min_s`, `belt_speed_headroom`, `belt_speed_min_mm_s`, `belt_speed_max_mm_s`, `belt_speed_hw_max_mm_s`, `belt_speed_deadband_mm_s`, `belt_speed_max_step_mm_s`, `belt_density_length_mm` | Adaptive belt speed law (§6, theory_basis.md) |
| `belt_accel_mm_s2`, `belt_ramp_s` | **Informational only** (kept deliberately): document a_nom and the ramp-settle invariant `max_step/a_nom ≈ 0.9 s < _GATE_CRITICAL_LEAD_S = 2.0 s`; not used in computation |
...
| Key | Purpose |
|---|---|
| `port`, `mjpeg_fps` | Web dashboard HTTP port / MJPEG frame rate (`interface.py`) |

**Look-alike keys, disambiguated:** `default_speed` (sim belt) ≠ `belt_speed_static_mm_s` (real belt); `accuracy_spawn_uv` (fake-object spawn points for test_accuracy/test_acceptance) ≠ `conveyor.accuracy_points_uv` (evaluate movement targets); `nominal_xy/z_speed` (rough ETA) ≠ `interpolator.*` (real timing model).
